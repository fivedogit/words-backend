...
public class MostLikedSnapshotter extends java.lang.Thread {
	...
	public MostLikedSnapshotter(int inc_num_hours, WordsMapper inc_mapper, DynamoDBMapperConfig inc_dynamo_config)
	{
		this.initialize();
		mapper = inc_mapper;
	...
	public void run()
	{
		System.out.println("=== " + super.getId() +  " Fired an MostLikedSnapshotter (web-wide, no url) thread.");
		try
		{
			...
			List<HPQSPLikeItem> scanResult = mapper.scan(HPQSPLikeItem.class, scanExpression, dynamo_config);
			...
			int temp_count = 0;
			for (HPQSPLikeItem ti : scanResult) {
			...
			newgvi.setName("most_liked_snapshot");
			...
			new_ts_gvi.setName("most_liked_snapshot_ts");
		...
		System.out.println("=== " + super.getId() +  " MostLikedSnapshotter Done.");
		return;
	}
